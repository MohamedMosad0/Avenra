...
export class ExternalDataProvider {
    ...
    async fetchNormalizedData() {
        ...
        try {
            ...
            const rawCategories = categoriesPayload?.data || [];
    ...
    async fetchPaginatedEndpoint(endpoint, limit = 50) {
        try {
            ...
            const results = Array.isArray(firstJson?.data) ? [...firstJson.data] : [];
            const totalPages = Number(firstJson?.metadata?.numberOfPages) || 1;
            if (totalPages > 1) {
                const pagePromises = [];
                for (let page = 2; page <= totalPages; page++) {
                    pagePromises.push(fetch(`${this.baseUrl}${endpoint}?limit=${limit}&page=${page}`, {
                        signal: AbortSignal.timeout(config.upstreamRequestTimeoutMs)
                    }));
                }
                ...
                for (let i = 0; i < pageResponses.length; i++) {
                    ...
                    const pageJson = (await pageResult.value.json());
