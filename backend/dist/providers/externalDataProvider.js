...
export class ExternalDataProvider {
    ...
    async fetchNormalizedData() {
        ...
        try {
            ...
            const categoriesPayload = (await categoriesRes.value.json());
            ...
            if (subcategoriesRes.status === "fulfilled" && subcategoriesRes.value.ok) {
                const subPayload = (await subcategoriesRes.value.json());
                rawSubcategories = subPayload?.data || [];
            }
            const productsPayload = (await productsRes.value.json());
            const rawProducts = productsPayload?.data || [];
