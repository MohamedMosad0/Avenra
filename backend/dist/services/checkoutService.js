...
import { CheckoutStorage } from "./checkoutStorage.js";
...
export class CheckoutService {
    ...
    constructor(catalogServiceOrStorageDir, customStorageOrDir) {
    ...
    createQuote(request, userId) {
        ...
        const deliveryMethod = request.deliveryMethod === "EXPRESS" ? "EXPRESS" : "STANDARD";
        ...
        for (const itemReq of request.items) {
            ...
            if (!product.isAvailable || product.availableQuantity < itemReq.quantity) {
                throw new AppError(409, "OUT_OF_STOCK", `Product '${product.title}' has insufficient stock (Requested: ${itemReq.quantity}, Available: ${product.availableQuantity}).`);
            }
    ...
    createOrder(request, idempotencyKey, userId, sessionTokenHash) {
        if (this.prunePersistedState()) {
            this.persistState();
        }
        if (!request.quoteId) {
    ...
    enforceQuoteLimit() {
        if (this.quotes.size <= MAX_ACTIVE_QUOTES)
            return;
        const excessQuotes = Array.from(this.quotes.entries())
            .sort(([, left], [, right]) => new Date(left.quote.quoteExpiry).getTime() - new Date(right.quote.quoteExpiry).getTime())
    ...
    reserveQuantities(items) {
        for (const item of items) {
            this.reservedQuantities.set(item.productId, (this.reservedQuantities.get(item.productId) || 0) + item.quantity);
        }
    }
    releaseQuantities(items) {
        for (const item of items) {
            const remaining = (this.reservedQuantities.get(item.productId) || 0) - item.quantity;
            if (remaining > 0)
                this.reservedQuantities.set(item.productId, remaining);
            else
                this.reservedQuantities.delete(item.productId);
        }
    }
    createScopeKey(userId, sessionTokenHash, idempotencyKey) {
