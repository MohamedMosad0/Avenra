...
async function runAuthTests() {
    ...
    try {
        ...
        if (signupData.user.password || signupData.user.passwordHash) {
        ...
        const malformedSignupData = (await malformedSignupRes.json());
        ...
        const malformedSigninData = (await malformedSigninRes.json());
        ...
        if (noHeaderRes.status !== 401)
            throw new Error("Expected missing auth header to return 401");
        const malformedHeaderRes = await fetch(`${baseUrl}/me`, {
            headers: { Authorization: "Basic dXNlcjpwYXNz" },
        });
        if (malformedHeaderRes.status !== 401)
            throw new Error("Expected Basic auth header to return 401 UNAUTHORIZED");
        console.log("  PASSED");
        ...
        const diskUser = diskUsers.find((u) => u.email === isolatedEmail);
        if (!diskUser)
            throw new Error("User was not saved to disk file!");
        ...
        if (diskSessions.some((session) => session.token || !session.tokenHash)) {
