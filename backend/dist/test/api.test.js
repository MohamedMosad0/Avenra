...
async function runTests() {
    ...
    function assertErrorResponse(data, expectedCode, stepName) {
    ...
    try {
        ...
        if (resHealth.status !== 200)
            throw new Error(`Health failed with status ${resHealth.status}`);
        const dataHealth = (await resHealth.json());
        if (dataHealth.status !== "ok")
            throw new Error(`Health expected { status: "ok" }, got ${JSON.stringify(dataHealth)}`);
        console.log("  PASSED");
        // 1. GET /v1/home
        console.log("Test 1: GET /v1/home");
        const resHome = await fetch(`${baseUrl}/home`);
        if (resHome.status !== 200)
            throw new Error(`Home failed with status ${resHome.status}`);
        ...
        if (resCat.status !== 200)
            throw new Error(`Categories failed with status ${resCat.status}`);
        ...
        if (resProd.status !== 200)
            throw new Error(`Products failed with status ${resProd.status}`);
        ...
        if (resCatFilter.status !== 200)
            throw new Error("Category filter query failed");
        const dataCatFilter = (await resCatFilter.json());
        if (!Array.isArray(dataCatFilter.products))
            throw new Error("Category filter did not return products array");
        for (const p of dataCatFilter.products) {
            if (p.categoryId !== sampleCategory.id)
                throw new Error(`Product ${p.id} categoryId ${p.categoryId} did not match filter ${sampleCategory.id}`);
        }
        // Search filter
        ...
        if (res404.status !== 404)
            throw new Error(`Expected 404 got ${res404.status}`);
        ...
        if (authRes.status !== 201)
            throw new Error("Checkout test authentication failed");
        ...
        if (emptyItemsQuote.status !== 400)
            throw new Error("Expected empty items quote to return 400");
        ...
        if (missingAddressQuote.status !== 400)
            throw new Error("Expected missing address quote to return 400");
        ...
        if (invalidProdQuote.status !== 404)
            throw new Error("Expected invalid product quote to return 404");
        ...
        if (outOfStockQuote.status !== 409)
            throw new Error("Expected out of stock quote to return 409");
        ...
        for (const invalidQty of invalidQuantities) {
            ...
            const dataInvalidQty = (await resInvalidQty.json());
        ...
        const quoteStandard = (await resStandard.json());
        ...
        const quoteExpress = (await resExpress.json());
        ...
        if (missingQuoteId.status !== 400)
            throw new Error("Expected missing quote ID order to return 400");
        ...
        if (crossUserOrder.status !== 400)
            throw new Error("Expected cross-user quote order to return 400");
        ...
        if (resOrderCard.status !== 201)
            throw new Error(`MOCK_CARD order failed with status ${resOrderCard.status}`);
        const orderCard = (await resOrderCard.json());
        ...
        if (resOrderCod.status !== 201)
            throw new Error(`CASH_ON_DELIVERY order failed with status ${resOrderCod.status}`);
        const orderCod = (await resOrderCod.json());
        ...
        const orderIdempRetry = (await resIdempRetry.json());
        ...
        if (resModifiedKey.status !== 409)
            throw new Error(`Expected 409 DUPLICATE_REQUEST on modified request, got ${resModifiedKey.status}`);
        ...
        const priceChangeQuote = (await priceChangeQuoteRes.json());
        ...
        try {
            ...
            if (orderPriceChangeRes.status !== 409)
                throw new Error(`Expected 409 PRICE_CHANGED, got ${orderPriceChangeRes.status}`);
        ...
        const stockProduct = dataProd.products.find((product) => product.id !== sampleProduct.id && product.availableQuantity > 0);
        if (!stockProduct)
            throw new Error("No independent in-stock product available for stock test");
        ...
        const fullStockQuote = (await fullStockQuoteRes.json());
        ...
        if (fullStockOrderRes.status !== 201)
            throw new Error("Stock reservation order failed");
        ...
        if (oversellQuoteRes.status !== 409)
            throw new Error(`Expected oversell quote to return 409, got ${oversellQuoteRes.status}`);
