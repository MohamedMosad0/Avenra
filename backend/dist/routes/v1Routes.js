...
// --- Auth Endpoints ---
// POST /v1/auth/signup
v1Router.post("/auth/signup", (req, res) => {
    try {
...
// POST /v1/auth/signin
v1Router.post("/auth/signin", (req, res) => {
    try {
...
// GET /v1/auth/me
v1Router.get("/auth/me", requireAuthentication, (_req, res) => {
    res.status(200).json({ user: res.locals.authUser });
});
// POST /v1/auth/revoke
v1Router.post("/auth/revoke", requireAuthentication, (_req, res) => {
...
v1Router.get("/products/:productId", (req, res, next) => {
...
v1Router.post("/checkout/quotes", requireAuthentication, (req, res, next) => {
    try {
...
// 6. POST /v1/orders
v1Router.post("/orders", requireAuthentication, (req, res, next) => {
    try {
        const idempotencyKey = req.header("Idempotency-Key");
        const order = checkoutService.createOrder(req.body, idempotencyKey, res.locals.authUser.id, hashToken(res.locals.authToken));
